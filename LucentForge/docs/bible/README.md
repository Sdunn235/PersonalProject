...
| `lucentforge_runtime_architecture_addendum_v1.md` | **Runtime architecture doctrine** (Stage 4.6R) — the application/session lifecycle: `SimulationKernel` (headless) + `WorldSession` (pygame-free graph) + `PresentationShell` (view), with `SimFrame` events, `Command` input, `RuntimeMode` states, and the **Ripple Kernel** pattern (player = another agent; world sims without a shell). Names the SimCore/Unreal port seam. | Cite for any run-loop, session-ownership, save/load-orchestration, input, or port work |
| `lucentforge_world_foundation_v1.md` | **World / Planetary foundation** (Arc B) — physical identity of the canonical home world: simulation-first geography, approaching-supercontinent state, central world spine, one canonical world vs. Genesis-generated worlds. Draft with Out-of-Scope register (§W8). | Primary authority for planetary/continental/world-scale truth |
| `lucentforge_planetary_tectonics_addendum_v1.md` | **Planetary tectonics** (§T1–§T10) — the 10-plate registry (stable IDs `PLATE-*`) and the *requirement* that every boundary be classified. Boundary types are **not** canon yet. | Cite for tectonic/plate/boundary work; pair with World Foundation |
| `lucentforge_world_map_scale_addendum_v1.md` | **World map & scale bridge** (§S1–§S9) — reconciles planetary authoring maps with the Panel doctrine. **Extends, does NOT supersede, `rooms_panels_addendum` §R1**: the Panel stays the one lived scale; the planetary map is a sparse authoring/indexing layer; `PlanetCoord` wraps `WorldPos`. | Cite for any planetary-coordinate, macro-map, or pixel→panel work |
| `lucentforge_climate_hydrology_addendum_v1.md` | **Climate / hydrology / biome derivation** (§C1–§C8) — the *order and rules* by which climate, rivers, and biomes derive from tectonics + elevation. Dependency doc; nothing canonized (§C8). | Cite when climate/biome data is derived or generated |
...
> **The World / Planetary docs EXTEND, they do not supersede, the Panel doctrine.** `world_map_scale_addendum` (§S) adds a planetary authoring/indexing layer *above* the Panel; `rooms_panels_addendum` §R1 (a Panel is one fixed 18×18 tile map, one scale everywhere) is unchanged. There is no world-map zoom; `PlanetCoord` wraps the existing `WorldPos`. The World docs are **drafts with Out-of-Scope registers** — canonical *structure* with known gaps (plate boundaries, planet scale), not finished canon. Generator implementation lives outside the Bible in `docs/genesis/`.
...
| World / Planetary (Arc B foundation) | §W1–§W8, §T1–§T10, §S1–§S9 (extends §R1), §C1–§C8 | `lucentforge_world_foundation_v1.md`, `lucentforge_planetary_tectonics_addendum_v1.md`, `lucentforge_world_map_scale_addendum_v1.md`, `lucentforge_climate_hydrology_addendum_v1.md`; + `docs/genesis/`, `docs/maps/` |
| Stage 5+ | TBD | TBD |
