...
def draw_observation_panel(surface, world_sim, sources, npc_list,
    ...
    for npc, ctrl in living:
        ...
        # Phase B+C legibility: show relocate target > strain > best-region hint.
        strain = ctrl.brain.chemicals.get("affinity_strain")
        ...
        if tgt:
            ...
            suffix, sfx_color = (f">{(dest.name if dest else tgt)[:5]}",
                                 settings.COLOR_WARNING)
        elif strain > 0.01:
            suffix, sfx_color = f"s{strain:.2f}", settings.COLOR_CRITICAL
        elif best:
            suffix, sfx_color = f"b{best[1]:+.1f}", settings.OBS_HINT_COLOR
        else:
            suffix, sfx_color = "", settings.OBS_LABEL_COLOR
        if suffix:
            surface.blit(font.render(suffix, True, sfx_color), (x + 134, y))
        y += _LINE
